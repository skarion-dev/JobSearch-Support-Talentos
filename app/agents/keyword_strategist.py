...
Runs ONCE per night on gpt-5.6-luna. Everything else in this system runs on
deepseek-v4-flash, but this is the one decision where being wrong is expensive:
a bad keyword set wastes the whole night's API budget and starves a candidate
of matches. It is one call, so a strong model is affordable here.
...
from app import db
from app.config import OPENCODE_API_KEY, OPENCODE_BASE_URL, STRATEGIST_MODEL
